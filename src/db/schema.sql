...
create policy "members_select_member" on public.project_members for select to authenticated using (
...
create policy "members_update_admin" on public.project_members for update to authenticated using (
  exists (
    select 1 from public.project_members m
    where m.project_id = project_id and m.user_id = auth.uid() and m.role in ('owner', 'admin')
  )
...
create policy "tickets_update_member" on public.tickets for update to authenticated using (
  reporter_id = auth.uid() or assignee_id = auth.uid() or exists (
    select 1 from public.project_members m
    where m.project_id = project_id and m.user_id = auth.uid() and m.role in ('owner', 'admin')
  )
);
